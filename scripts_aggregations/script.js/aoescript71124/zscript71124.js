...
//find if the effects effect each unit classes/id
//if so add the unit's pgbid to a new array that is a property on the tech object

for (let tech of techs){
  if(tech.effects){
    for (effect of tech.effects){
      if(effect.select.id){
        effect.select.id.forEach((id) => {
          if(units.filter((unit) => unit.id )){
