...
const mongoConnect = async () => {
  try {
    ...
    collection = await db.collection("AOE43rdCollection");
...
app.post("/api/getSelectAge", async (request, response) => {
  try {
    ...
    const itemsAges = await collection
      .find(
        {
          baseId: reverseSelectedText,
          civs: { $in: [request.body.selectCiv.toLowerCase()] },
        },
    ...
    if (valuesOnly.includes(4)) {
      valuesOnly = valuesOnly;
    } else {
      let i = 1;
      while (!valuesOnly.includes(4)) {
...
app.post("/api/getSelectTechs", async (request, response) => {
  try {
    ...
    console.log("tech request", request.body);
    let test = await collection.findOne({
      //name: request.body.selectText.replace(" ", "-").toLowerCase(),
      name: reverseFormat(request.body.selectText),
      civs: { $in: [request.body.selectCiv.toLowerCase()] },
      age: Number(request.body.selectAge),
    });

    const filter2 = {
      civs: { $in: [request.body.selectCiv] },
      effectedUnitIds: {
        // $in: [request.body.selectText.toLowerCase().replace(" ", "-")],
        $in: [reverseFormat(request.body.selectText)],
      },
      minAge: { $lte: Number(request.body.selectAge) },
    };
    console.log(filter2);
...
app.post("/api/calculate", async (request, response) => {
  try {
    ...
    let unitObject1 = await collection.findOne({
      ...
      baseId: reverseFormat(request.body.unit1),
      civs: { $in: [request.body.civ1] },
      age: Number(request.body.age1),
    });
    let unitObject2 = await collection.findOne({
      // name: request.body.unit2,
      baseId: reverseFormat(request.body.unit2),
      civs: { $in: [request.body.civ2] },
    ...
    if (request.body.techs1) {
      for (let i = 0; i < request.body.techs1.length; i++) {
        let techObjects1 = await collection2.findOne({
          _id: new ObjectId(request.body.techs1[i]),
        });
    ...
    if (request.body.techs2) {
      ...
      for (let i = 0; i < request.body.techs2.length; i++) {
        let techObjects2 = await collection2.findOne({
          _id: new ObjectId(request.body.techs2[i]), //********** */
        });
    ...
    if (unitObject1.armor.length == 0) {
      //unit1 add armor values and type if object does not have any
      unitObject1.armor = [];
      unitObject1.armor[0] = {};
    ...
    //add
      ...
      unitObject1.armor.findIndex((x) => x.type === "ranged") > 0 &&
      !unitObject1.armor.findIndex((x) => x.type === "melee") < 0
    ) {
      unitObject1.armor = [];
      unitObject1.armor[1] = {};
      unitObject1.armor[1].type = "melee";
    ...
    //
    if (unitObject2.armor.length == 0) {
      //unit2 add armor values and type if object does not have any
      unitObject2.armor = [];
      unitObject2.armor[0] = {};
      ...
      unitObject2.armor.findIndex((x) => x.type === "ranged") > 0 &&
      !unitObject2.armor.findIndex((x) => x.type === "melee") < 0
    ) {
      unitObject2.armor = [];
      unitObject2.armor[1] = {};
      unitObject2.armor[1].type = "melee";
    ...
    let object1IndexOfRangedArmor = unitObject1.armor.findIndex(
      (x) => x.type === "ranged"
    );
    let object1IndexOfMeleeArmor = unitObject1.armor.findIndex(
      (x) => x.type === "melee"
    );
    let object2IndexOfRangedArmor = unitObject2.armor.findIndex(
      (x) => x.type === "ranged"
    );
    let object2IndexOfMeleeArmor = unitObject2.armor.findIndex(
      (x) => x.type === "melee"
    );

    for (let eachTechObject of tech1Array) {
      //iterates through each select tech
      for (let effectObject of eachTechObject.effects) {
        //iterates through each tech effect

        if (
          effectObject.property == "rangedAttack" &&
          unitObject1.weapons[0].type == "ranged"
        ) {
          //dont need to iterate through unitObject weapons because a unit only has one weapon type
          unitObject1.weapons[0].damage = maths(
            effectObject.effect,
            unitObject1.weapons[0].damage,
            effectObject.value
          );
        } else if (
          effectObject.property == "meleeAttack" &&
          unitObject1.weapons[0].type == "melee"
        ) {
          unitObject1.weapons[0].damage = maths(
            effectObject.effect,
            unitObject1.weapons[0].damage,
          ...
          effectObject.property == "rangedArmor" &&
          unitObject1.armor[object1IndexOfRangedArmor].type === "ranged"
        ) {
          unitObject1.armor[object1IndexOfRangedArmor].value = maths(
            effectObject.effect,
            unitObject1.armor[object1IndexOfRangedArmor].value,
            effectObject.value
          );
        } else if (
          effectObject.property == "meleeArmor" &&
          unitObject1.armor[object1IndexOfMeleeArmor].type == "melee"
        ) {
          unitObject1.armor[object1IndexOfMeleeArmor].value = maths(
            effectObject.effect,
            unitObject1.armor[object1IndexOfMeleeArmor].value,
            effectObject.value
          );
        } else if (effectObject.property == "hitpoints") {
          unitObject1.hitpoints = maths(
            effectObject.effect,
            unitObject1.hitpoints,
            effectObject.value
          );
        } else if (effectObject.property == "attackSpeed") {
          unitObject1.weapons[0].speed = maths(
            effectObject.effect,
            unitObject1.weapons[0].speed,
            effectObject.value
          );
    ...
    for (let eachTechObject of tech2Array) {
      //iterates through each select tech
      for (let effectObject of eachTechObject.effects) {
        //iterates through each tech effect
        ...
        if (
          effectObject.property == "rangedAttack" &&
          unitObject2.weapons[0].type == "ranged"
        ) {
          //dont need to iterate through unitObject weapons because a unit only has one weapon type
          unitObject2.weapons[0].damage = maths(
            effectObject.effect,
            unitObject2.weapons[0].damage,
            effectObject.value
          );
        } else if (
          effectObject.property == "meleeAttack" &&
          unitObject2.weapons[0].type == "melee"
        ) {
          unitObject2.weapons[0].damage = maths(
            effectObject.effect,
            unitObject2.weapons[0].damage,
          ...
          effectObject.property == "rangedArmor" &&
          unitObject2.armor[object2IndexOfRangedArmor].type === "ranged"
        ) {
          unitObject2.armor[object2IndexOfRangedArmor].value = maths(
            effectObject.effect,
            unitObject2.armor[object2IndexOfRangedArmor].value,
            effectObject.value
          );
        } else if (
          effectObject.property == "meleeArmor" &&
          unitObject2.armor[object2IndexOfMeleeArmor].type == "melee"
        ) {
          unitObject2.armor[object2IndexOfMeleeArmor].value = maths(
            effectObject.effect,
            unitObject2.armor[object2IndexOfMeleeArmor].value,
            effectObject.value
          );
        } else if (effectObject.property == "hitpoints") {
          unitObject2.hitpoints = maths(
            effectObject.effect,
            unitObject2.hitpoints,
            effectObject.value
          );
        } else if (effectObject.property == "attackSpeed") {
          unitObject2.weapons[0].speed = maths(
            effectObject.effect,
            unitObject2.weapons[0].speed,
            effectObject.value
          );
        } else if (effectObject.property == "siegeAttack") {
          unitObject2.weapons[0].damage = maths(
    ...
    const numberOfUnits1 = request.body.numberOfUnits1
      ? request.body.numberOfUnits1
      : 1;
    const teamOneHitpoints = unitObject1.hitpoints * numberOfUnits1;
    const teamOneDamage = unitObject1.weapons[0]
      ? unitObject1.weapons[0].damage
      : 0;
    const teamOneAttackSpeed = unitObject1.weapons[0]
      ? unitObject1.weapons[0].speed
      : 0;
    //const teamOneMeleeArmor = unitObject1.armor[object1IndexOfMeleeArmor].value
    const teamOneRangedArmor =
      unitObject1.armor[object1IndexOfRangedArmor].value;
    let teamOneWeaponModifier = 0;
    let teamOneRelevantArmor = 0;

    const numberOfUnits2 = request.body.numberOfUnits2
      ? request.body.numberOfUnits2
      : 1;
    const teamTwoHitpoints = unitObject2.hitpoints * numberOfUnits2;
    const teamTwoDamage = unitObject2.weapons[0]
      ? unitObject2.weapons[0].damage
      : 0;
    const teamTwoAttackSpeed = unitObject2.weapons[0]
      ? unitObject2.weapons[0].speed
      : 0;
    //const teamTwoMeleeArmor = unitObject2.armor[object2IndexOfMeleeArmor].value
    const teamTwoRangedArmor =
      unitObject2.armor[object2IndexOfRangedArmor].value;
    ...
    if (
      unitObject1.weapons.length !== 0 &&
      unitObject1.weapons[0].modifiers.length !== 0 &&
      unitObject1.weapons[0].modifiers[0].target.class[0].every((unitClass) =>
        unitObject2.displayClasses[0].toLowerCase().includes(unitClass)
      )
    ) {
    ...
    if (
      unitObject2.weapons.length !== 0 &&
      unitObject2.weapons[0].modifiers.length !== 0 &&
      unitObject2.weapons[0].modifiers[0].target.class[0].every((unitClass) =>
        unitObject1.displayClasses[0].toLowerCase().includes(unitClass)
      )
    ) {
    ...
    //final calculation
    let teamOneTrueDamage =
      Math.round(
        (((teamOneDamage + teamOneWeaponModifier - teamTwoRelevantArmor) *
          numberOfUnits1) /
          teamOneAttackSpeed) *
          100
      ) / 100;
    ...
    let teamTwoTrueDamage =
      Math.round(
        (((teamTwoDamage + teamTwoWeaponModifier - teamOneRelevantArmor) *
          numberOfUnits2) /
          teamTwoAttackSpeed) *
          100
      ) / 100;
    ...
    const teamOneTimeToKillTeamTwo =
      Math.round((teamTwoHitpoints / teamOneTrueDamage) * 100) / 100;
    const teamTwoTimeToKillTeamOne =
      Math.round((teamOneHitpoints / teamTwoTrueDamage) * 100) / 100;
    ...
    if (teamOneTimeToKillTeamTwo < teamTwoTimeToKillTeamOne) {
      ...
      result = {
        tieStatement:
          "No blood was shed on this day as there is a tie. Please try another combination.",
      };
